  ...
  return (
    <Dialog open={isOpen} onOpenChange={(val) => !val && onClose()}>
      <DialogContent className={cn(
        ...
        <button 
          onClick={onClose}
          className="absolute right-4 top-4 z-[150] p-2 bg-black text-white hover:bg-[#01a3a4] transition-all border border-white/10 shadow-2xl"
        >
          <X className="h-7 w-7" />
        </button>
        ...
        <div className="flex flex-col h-full max-h-[95vh] relative no-scrollbar">
          {step === 'FORM' ? (
            <div className="flex flex-col md:flex-row h-full overflow-hidden">
              ...
              <div className={cn(
                "flex-grow p-8 md:p-20 pt-20 md:pt-28 space-y-10 md:space-y-14 bg-white overflow-y-auto relative no-scrollbar",
                !isMobile && "md:w-[450px]"
              )}>
                <div className="space-y-4">
                  ...
                  <DialogDescription className="text-[12px] md:text-[14px] font-black text-gray-400 uppercase tracking-[0.4em] pl-8">
                    SECURE PURCHASE ARCHIVE
                  </DialogDescription>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-10 md:space-y-12 pt-6">
                  <div className="grid grid-cols-2 gap-8 md:gap-12">
                    <div className="space-y-4">
                      <label className="text-[12px] md:text-[13px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-4">
                        <Ruler className="h-6 w-6 text-[#01a3a4]" /> SELECT SIZE
                      </label>
                      <div className="flex flex-wrap gap-3">
                        {product?.sizes?.length > 0 ? product.sizes.map((size: string) => (
                          <button
                            ...
                            className={cn(
                              "px-6 py-3 border-2 text-[13px] font-black uppercase transition-all min-w-[70px]",
                    ...
                    <div className="space-y-4">
                      <label className="text-[12px] md:text-[13px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-4">
                        <Hash className="h-6 w-6 text-[#01a3a4]" /> QUANTITY
                      </label>
                      <input 
                        type="number"
                        min="1"
                        ...
                        className="w-full bg-gray-50 border-2 border-gray-200 rounded-none h-16 px-6 text-[18px] font-black tracking-widest focus:outline-none focus:border-[#01a3a4] text-black"
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-8 md:space-y-10">
                    <div className="space-y-4">
                      <label className="text-[12px] md:text-[13px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-4">
                        <User className="h-6 w-6 text-[#01a3a4]" /> FULL NAME
                      </label>
                      <input 
                        required
                        value={formData.name}
                        ...
                        className="w-full bg-gray-50 border-2 border-gray-200 rounded-none h-20 px-8 text-[16px] font-black uppercase tracking-widest focus:outline-none focus:border-[#01a3a4] text-black shadow-lg"
                      />
                    </div>

                    <div className="space-y-4">
                      <label className="text-[12px] md:text-[13px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-4">
                        <Phone className="h-6 w-6 text-[#01a3a4]" /> PHONE NUMBER
                      </label>
                      <input 
                        required
                        type="tel"
                    ...
                    <div className="space-y-4">
                      <label className="text-[12px] md:text-[13px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-4">
                        <MapPin className="h-6 w-6 text-[#01a3a4]" /> DELIVERY ADDRESS
                      </label>
                      <textarea 
                        required
                        ...
                        className="w-full bg-gray-50 border-2 border-gray-200 rounded-none p-8 text-[16px] font-black uppercase tracking-widest min-h-[150px] focus:outline-none focus:border-[#01a3a4] text-black shadow-lg no-scrollbar"
                      />
                    </div>
                  </div>
                  ...
                  <Button 
                    type="submit" 
                    className="w-full bg-[#01a3a4] hover:bg-black text-white h-24 md:h-28 font-black uppercase tracking-[0.8em] rounded-none shadow-2xl text-[20px] md:text-[22px] border-none transition-all active:scale-95 mt-10"
                  >
                    অর্ডার নিশ্চিত করুন
                  </Button>
                </form>
            ...
            <div className="w-full p-20 text-center space-y-12 flex flex-col justify-center bg-white items-center min-h-[600px]">
              <div className="relative">
                <div className="w-32 h-32 bg-[#01a3a4]/5 rounded-full flex items-center justify-center mx-auto mb-10 border-[8px] border-[#01a3a4] shadow-2xl animate-in zoom-in-50 duration-700">
              ...
              <div className="space-y-10">
                <DialogTitle className="text-5xl md:text-8xl font-black text-black uppercase tracking-tighter leading-none font-headline animate-in slide-in-from-bottom-2 duration-1000">
                  THANK YOU
                </DialogTitle>
                ...
                <div className="py-4 px-12">
                  <p className="text-[20px] md:text-[24px] font-bold text-black font-headline leading-relaxed">
